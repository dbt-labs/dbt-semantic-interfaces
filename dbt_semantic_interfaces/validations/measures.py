...
class SimpleMetricWithoutSourceHasExtraFieldsRule(
    ...
    def _get_sourceless_metric_fields_from_metric(metric: Metric) -> List[str]:
        ...
        all_sourceless_only_fields = (
            SimpleMetricWithoutSourceHasExtraFieldsRule.SOURCELESS_REQUIRED_FIELDS
            + SimpleMetricWithoutSourceHasExtraFieldsRule.SOURCELESS_OPTIONAL_FIELDS
        )
        return [field for field in all_sourceless_only_fields if getattr(metric, field, None) is not None]
    ...
    def _get_required_fields_missing_from_sourceless_metric(metric: Metric) -> List[str]:
        ...
        return [
            field
            for field in SimpleMetricWithoutSourceHasExtraFieldsRule.SOURCELESS_REQUIRED_FIELDS
    ...
    def validate_manifest(semantic_manifest: SemanticManifestT) -> Sequence[ValidationIssue]:  # noqa: D
        ...
        for metric in semantic_manifest.metrics:
            if metric.type != MetricType.SIMPLE:
                continue
            has_source = SimpleMetricWithoutSourceHasExtraFieldsRule._has_separate_source(metric)
            if has_source:
                illegal_sourceless_only_fields = (
                    SimpleMetricWithoutSourceHasExtraFieldsRule._get_sourceless_metric_fields_from_metric(metric)
                )
                if len(illegal_sourceless_only_fields) > 0:
                    issues.append(
                        ValidationError(
                            context=MetricContext(
                                file_context=FileContext.from_metadata(metadata=metric.metadata),
                            ...
                            message=(
                                ...
                                f"have the following fields: {illegal_sourceless_only_fields}."
                            ),
                        )
                    )
                continue
            ...
            missing_required_sourceless_fields = (
                SimpleMetricWithoutSourceHasExtraFieldsRule._get_required_fields_missing_from_sourceless_metric(metric)
            )
